...
ClinicOS uses PostgreSQL (via Supabase) with Prisma ORM. The schema includes 11 models designed for clinic appointment management, visit documentation, and audit trail support.
...
| demoPassword | String?  | Optional            | Readable password, set only for demo accounts listed on the login page |
...
| _[all visit note fields]_ | Various  | Nullable      | Snapshot of note     |
...
Security-focused audit trail for accountability and traceability (demonstration; not security-certified).
...
The AI assistant has no database access and no tools. Before each reply, the server runs one read-only query for today's appointments (`lib/ai/clinic-context.ts`) and turns it into a short text summary for the model:

- Selected columns: scheduled time, duration, status, type, and provider name (shown to front desk only)
- A provider's query is filtered to their own `providerId`; front desk sees the whole clinic
- Patient names, contact details and visit notes are never selected

Everything else the assistant knows comes from its system prompt (roles, statuses and workflows).
...
7. **Provider Limit**: At most 5 active providers, checked when one is added or reactivated
8. **Login-Page Demo Accounts**: `User.demoPassword` is set only when front desk lists an account, and is updated whenever that account's password changes
...
5. **Fewer round trips**: Each query through the transaction pooler takes several network round trips, so the dashboard derives its summary counts from one group-by, and functions run in the database's region
...
20260914063000_add_user_demo_password/
```

### Data Migrations

Changes to how existing data is _encoded_ (not to the schema) live in `scripts/`:
