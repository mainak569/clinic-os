...
async function main() {
  ...
  await prisma.user.create({
    data: {
      ...
      demoPassword: "FrontDesk123!", // listed on the login page
  ...
  const drSmithUser = await prisma.user.create({
    data: {
      ...
      demoPassword: "DrSmith123!", // listed on the login page
      role: Role.PROVIDER,
      lastLogin: new Date(),
    },
  });
  ...
  const drJohnsonUser = await prisma.user.create({
    data: {
      ...
      demoPassword: "DrJohnson123!", // listed on the login page
      role: Role.PROVIDER,
      lastLogin: new Date(),
    },
  });
  ...
  console.log(
    `Created ${drSmithSlots.length + drJohnsonSlots.length} availability slots`
  );
    ...
    if (appointment) {
      await prisma.visitNote.create({
        data: {
          ...
          historyOfPresent:
            "Patient reports feeling well overall. No acute concerns.",
          physicalExam:
            "Well-appearing adult. HEENT normal, CV regular rate and rhythm, lungs clear, abdomen soft.",
          ...
          followUpInstructions:
            "Continue healthy diet and exercise. Schedule mammogram.",
