...
const mockPrismaAppointmentHistoryCreate =
...
const mockDescribeUnavailability = jest.fn() as jest.MockedFunction<any>;
jest.mock("@/lib/services/availability.service", () => ({
  availabilityService: {
    isProviderAvailable: mockIsProviderAvailable,
    describeUnavailability: mockDescribeUnavailability,
  },
}));
...
describe("Appointment Service - State Machine", () => {
  ...
  describe("createAppointment", () => {
    ...
    it("should reject if provider not available, saying why", async () => {
      mockIsProviderAvailable.mockResolvedValue(false);
      mockDescribeUnavailability.mockResolvedValue(
        "This 30-minute visit would run from 9:00 AM to 9:30 AM, outside the provider's hours on Mondays (1:00 PM–4:00 PM)."
      );
      ...
      ).rejects.toThrow("outside the provider's hours on Mondays");
    });
  ...
  describe("timing rules", () => {
    ...
    it("refuses to confirm once the start time has passed", async () => {
      mockPrismaAppointmentFindUnique.mockResolvedValue(
        appointmentAt("REQUESTED", minutes(-10))
      );
    ...
    it("opens check-in an hour before the visit and closes it when the visit ends", async () => {
      ...
      mockPrismaAppointmentFindUnique.mockResolvedValue(
        appointmentAt("CONFIRMED", minutes(90))
      );
      await expect(
        appointmentService.checkInAppointment("appt1", "user1")
      ...
      mockPrismaAppointmentFindUnique.mockResolvedValue(
        appointmentAt("CONFIRMED", minutes(-45))
      );
      await expect(
        appointmentService.checkInAppointment("appt1", "user1")
      ...
      mockPrismaAppointmentFindUnique.mockResolvedValue(
        appointmentAt("CONFIRMED", minutes(30))
      );
      await expect(
        appointmentService.checkInAppointment("appt1", "user1")
    ...
    it("refuses to complete a visit before its start time", async () => {
      mockPrismaAppointmentFindUnique.mockResolvedValue(
        appointmentAt("CHECKED_IN", minutes(20))
      );
    ...
    it("refuses to cancel a confirmed appointment after its start, but clears an unconfirmed one", async () => {
      ...
      mockPrismaAppointmentFindUnique.mockResolvedValue(
        appointmentAt("CONFIRMED", minutes(-5))
      );
      await expect(
      ...
      mockPrismaAppointmentFindUnique.mockResolvedValue(
        appointmentAt("REQUESTED", minutes(-5))
      );
      await expect(
    ...
    it("refuses to reschedule once the patient has checked in", async () => {
      mockPrismaAppointmentFindUnique.mockResolvedValue(
        appointmentAt("CHECKED_IN", minutes(-5))
      );
    ...
    it("refuses when someone else changed the status since it was read", async () => {
      mockPrismaAppointmentFindUnique.mockResolvedValue(
        appointmentAt("REQUESTED", minutes(60))
      );
