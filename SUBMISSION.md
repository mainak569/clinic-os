...
It's a demonstration project: the architecture and security patterns follow HIPAA-oriented principles, but the app has not undergone formal compliance validation and is not intended for real patient data. See [Scope & Future Work](#scope--future-work) for what a production deployment would still need.
...
- 179 automated tests covering business logic, authorization, security boundaries, and database workflows
...
| 8   | HIPAA-Oriented Audit Logging | Complete | Append-only audit trail for patient, appointment, visit note, and provider changes with user context (demonstration; not compliance-certified) |
...
### Audit Logging
- HIPAA-oriented audit trail (demonstration purposes) of patient record views and all changes to patients, appointments, visit notes and providers, with user, IP and user agent
...
- **Unit Tests**: 117/117 passing — validation schemas, appointment service business logic (state machine, timing rules, concurrent changes), authorization helpers, clinic-time conversions, analytics provider isolation, appointment-details view, API route and cron authentication, alert de-duplication and ownership, sign-in lockout
...
**Total: 179/179 tests passing.** Unit tests mock the database; integration tests run against a local PostgreSQL test database (`clinicos_test`) with mocked authentication, never against Supabase.

## Build & Deployment Status

- **Build**: Passes (`npm run build`) — **Type checking**: no errors (`npx tsc --noEmit`) — **Linting**: passes (`npm run lint`)
- **Tests**: 179/179 passing (`npm test`)
- **Deployment**: Vercel-ready; set `CLINIC_TIMEZONE` and `CRON_SECRET` in the environment
...
**Total**: Approximately 20-26 hours, across 11 sessions (see [docs/plan.md](./docs/plan.md) for the phase-by-phase breakdown) — roughly 16-22 hours building the feature set (Sessions 1-10) and 4 hours on the consistency audit above (Session 11).
...
2. **Email/SMS notifications** — alerts are shown in the dashboard only; adding transactional email is the top item under [What Would You Do Next](#what-would-you-do-next-with-another-27-30-hours).
...
## What Would You Do Next, With Another 27-30 Hours?

The first five priorities address the scope items above, ordered by what would matter most to a real clinic rather than by visual polish. Priority 6 is a new feature — a RAG-based assistant — estimated separately at 15-18 hours given the added infrastructure (a vector store, a retrieval pipeline, and prompt/guardrail work), on top of the 12 hours for priorities 1-5.
...
- Session inactivity timeout and password complexity rules for new provider accounts
...
- Component tests for the appointment dialogs
- CI running lint, typecheck, unit and integration tests on every push

### Priority 3: Scheduling and workflow improvements (3 hours)
...
- A read-only front-desk page for the AuditLog table, filterable by user, record and date

### Priority 6: RAG-based assistant, signed-in users only (15-18 hours)
- Gated behind the existing session check — no anonymous access, same as every other route
- Retrieval scoped to what the signed-in user could already see through the rest of the app, using the same `canAccessProviderData`/`canAccessPatient` checks the rest of the app uses — not a separate, unaudited access path
- `pgvector` on the existing Supabase Postgres rather than new infrastructure
- Guardrails: refuse to answer from anything outside the retrieved, permission-filtered context; log every question and the retrieved context to `AuditLog`, consistent with the existing audit trail
...
- **Testing**: 179 tests passing (117 unit + 62 integration). Integration tests need a local PostgreSQL `clinicos_test` database
